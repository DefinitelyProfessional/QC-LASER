...
#include <GL/gl.h>
// Clean up Windows macro pollution that breaks MicroTeX identifiers
#ifdef TRANSPARENT
#undef TRANSPARENT
#endif
#ifdef OPAQUE
...
struct OpenGLTexture {
    ...
    // Destructor automatically frees the GPU memory when the shared_ptr drops to 0.
    ~OpenGLTexture() {
        if (ID != 0) {
...
struct FormulaTexture {
    ...
    ImTextureID TextureId() const {
        // Cast the raw OpenGL GLuint to ImTextureID (void*) for Dear ImGui
        return Texture ? reinterpret_cast<ImTextureID>(static_cast<uintptr_t>(Texture->ID)) : (ImTextureID)0;
    }
};
