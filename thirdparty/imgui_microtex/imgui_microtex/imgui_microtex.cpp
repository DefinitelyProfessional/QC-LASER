...
// DECOUPLED HELLO IMGUI, replaced with custom implementation for opengl backend
...
FormulaTexture ToTexture(const RenderedFormula& formula) {
    ...
    // Create the shared pointer to manage lifetime
    tex.Texture = std::make_shared<OpenGLTexture>();
    
    // Generate and bind the OpenGL texture
    glGenTextures(1, &tex.Texture->ID);
    glBindTexture(GL_TEXTURE_2D, tex.Texture->ID);
    
    // Set texture scaling filters (Linear for smooth math edges)
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    
    // Upload the raw CPU pixel buffer to the GPU
    // formula.Pixels.data() contains exactly formula.Width * formula.Height * 4 bytes (RGBA)
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, formula.Width, formula.Height, 0, GL_RGBA, GL_UNSIGNED_BYTE, formula.Pixels.data());
